...
def run_test(model: YamlBertModel, vocab: Vocabulary, config: YamlBertConfig,
    ...
    # v9 whole-word masking: mask_pos is a LOGICAL position (index into nodes).
    # We need to mask ALL subword positions that came from that logical node.
    # The dataset already laid them out contiguously and recorded logical_ids.
    ...
    # YamlBertModel v9 returns (logits, doc_vec) where logits is
    # (B, L_max, V_atomic) — indexed by LOGICAL position.
    logits = out[0]
